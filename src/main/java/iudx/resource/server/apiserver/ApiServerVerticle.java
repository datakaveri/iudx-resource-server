...
import iudx.resource.server.apiserver.metering.RequestMonitoring;
...
public class ApiServerVerticle extends AbstractVerticle {
	 ...
	 * exposed through service discovery, start an HTTPs server at port 8443 or an
	...
	public void start() throws Exception {
		...
		// handler for any * api call : call your method.
		...
		router.get(NGSILD_ENTITIES_URL).handler(entityValidationHandler).handler(AuthHandler.create(vertx))
		...
		router.get(NGSILD_ENTITIES_URL + "/:domain/:userSha/:resourceServer/:resourceGroup/:resourceName")
				.handler(latestValidationHandler).handler(AuthHandler.create(vertx))
		...
		router.post(NGSILD_POST_QUERY_PATH).consumes(APPLICATION_JSON).handler(postValidationHandler)
				.handler(AuthHandler.create(vertx)).handler(this::handlePostEntitiesQuery)
				.failureHandler(validationsFailureHandler);

		ValidationHandler temporalValidationHandler = new ValidationHandler(vertx, RequestType.TEMPORAL);
		router.get(NGSILD_TEMPORAL_URL).handler(temporalValidationHandler).handler(AuthHandler.create(vertx))
				.handler(this::handleTemporalQuery).failureHandler(validationsFailureHandler);

//    router.post(METERING_DB_UPDATE_TEST_URL).handler(AuthHandler.create(vertx))
//    	.handler(this::handleSubscriptions);

		router.post(NGSILD_SUBSCRIPTION_URL).handler(AuthHandler.create(vertx)).handler(this::handleSubscriptions);
		// append sub
		router.patch(NGSILD_SUBSCRIPTION_URL + "/:domain/:userSHA/:alias").handler(AuthHandler.create(vertx))
				.handler(this::appendSubscription);
		// update sub
		router.put(NGSILD_SUBSCRIPTION_URL + "/:domain/:userSHA/:alias").handler(AuthHandler.create(vertx))
				.handler(this::updateSubscription);
		// get sub
		router.get(NGSILD_SUBSCRIPTION_URL + "/:domain/:userSHA/:alias").handler(AuthHandler.create(vertx))
				.handler(this::getSubscription);
		// delete sub
		router.delete(NGSILD_SUBSCRIPTION_URL + "/:domain/:userSHA/:alias").handler(AuthHandler.create(vertx))
	...
	private void handleLatestEntitiesQuery(RoutingContext routingContext) {
		...
		if (!params.isEmpty()) {
			RuntimeException ex = new RuntimeException("Query parameters are not allowed with latest query");
		...
		String id = domain + "/" + userSha + "/" + resourceServer + "/" + resourceGroup + "/" + resourceName;
		...
		filtersFuture.onComplete(filtersHandler -> {
			if (filtersHandler.succeeded()) {
				...
				handleResponse(response, ResponseType.BadRequestData, filtersHandler.cause().getMessage());
			}
		});
	}
	...
	private void handleEntitiesQuery(RoutingContext routingContext) {
		...
		validationResult.onComplete(validationHandler -> {
			if (validationHandler.succeeded()) {
				...
				Future<List<String>> filtersFuture = catalogueService
						.getApplicableFilters(json.getJsonArray("id").getString(0));
				/* HTTP request instance/host details */
				String instanceID = request.getHeader(HEADER_HOST);
				json.put(JSON_INSTANCEID, instanceID);
				LOGGER.debug("Info: IUDX query json;" + json);
...
//    /* Handles HTTP response from server to client */
	...
	private JsonObject createJson(RoutingContext routingContext) {
		...
		context.put("rosourceId", resource_ID);
		context.put("email", email);
		context.put("api", api);
		context.put("time", current_Time_Stamp);
	...
	private void handleSubscriptions(RoutingContext routingContext) {
		...
		String subscrtiptionType = subHeader != null && subHeader.contains(SubsType.STREAMING.getMessage())
		...
		if (requestBody.containsKey(SUB_TYPE)) {
			...
			subsReq.onComplete(subHandler -> {
				if (subHandler.succeeded()) {
					...
					handleSuccessResponse(response, ResponseType.Created.getCode(), subHandler.result().toString());
				} else {
		...
		if (requestJson != null && requestJson.containsKey(SUB_TYPE)) {
			...
			subsReq.onComplete(subHandler -> {
				if (subHandler.succeeded()) {
					...
					handleSuccessResponse(response, ResponseType.Ok.getCode(), subHandler.result().toString());
				} else {
					LOGGER.error("Fail: Bad request");
					processBackendResponse(response, subHandler.cause().getMessage());
				}
	...
	private void unbindQueue2Exchange(RoutingContext routingContext) {
		...
		if (request.headers().contains(HEADER_TOKEN)) {
			...
			authenticator.tokenInterospect(requestJson.copy(), authenticationInfo, authHandler -> {
				if (authHandler.succeeded()) {
					Future<JsonObject> brokerResult = managementApi.unbindQueue2Exchange(requestJson, databroker);
					brokerResult.onComplete(brokerResultHandler -> {
						if (brokerResultHandler.succeeded()) {
	...
	public void deleteAdapter(RoutingContext routingContext) {
		...
		brokerResult.onComplete(brokerResultHandler -> {
			if (brokerResultHandler.succeeded()) {
				...
				handleSuccessResponse(response, ResponseType.Ok.getCode(), brokerResultHandler.result().toString());
			} else {
				LOGGER.error("Fail: Bad request;" + brokerResultHandler.cause().getMessage());
				processBackendResponse(response, brokerResultHandler.cause().getMessage());
			}
	 ...
	 * 
	 * @param routingContext routingContext Note: This is too frequent an operation
	 *                       to have info or error level logs
	 */
	...
	public void publishDownstreamIssue(RoutingContext routingContext) {
		...
		if (request.headers().contains(HEADER_TOKEN)) {
			...
			authenticator.tokenInterospect(requestJson.copy(), authenticationInfo, authHandler -> {
				if (authHandler.succeeded()) {
					Future<JsonObject> brokerResult = managementApi.publishDownstreamIssues(requestJson, databroker);
					brokerResult.onComplete(brokerResultHandler -> {
						if (brokerResultHandler.succeeded()) {
	...
	private void handleResponse(HttpServerResponse response, ResponseType responseType, String message) {
		response.putHeader(CONTENT_TYPE, APPLICATION_JSON).setStatusCode(responseType.getCode())
	...
	private JsonObject generateResponse(ResponseType responseType) {
		...
		return new RestResponse.Builder().withType(type).withTitle(ResponseType.fromCode(type).getMessage())
	...
	private JsonObject generateResponse(ResponseType responseType, String message) {
		int type = responseType.getCode();
		return new RestResponse.Builder().withType(type).withTitle(ResponseType.fromCode(type).getMessage())
				.withMessage(message).build().toJson();
	...
	private Optional<MultiMap> getQueryParams(RoutingContext routingContext, HttpServerResponse response) {
		...
		try {
			...
			// Internally + sign is dropped and treated as space, replacing + with %2B do
			// the trick
			String uri = routingContext.request().uri().toString().replaceAll("\\+", "%2B");
			Map<String, List<String>> decodedParams = new QueryStringDecoder(uri, HttpConstants.DEFAULT_CHARSET, true,
					1024, true).parameters();
			for (Map.Entry<String, List<String>> entry : decodedParams.entrySet()) {
				LOGGER.debug("Info: param :" + entry.getKey() + " value : " + entry.getValue());
				queryParams.add(entry.getKey(), entry.getValue());
			}
		} catch (IllegalArgumentException ex) {
			response.putHeader(CONTENT_TYPE, APPLICATION_JSON).setStatusCode(ResponseType.BadRequestData.getCode())
