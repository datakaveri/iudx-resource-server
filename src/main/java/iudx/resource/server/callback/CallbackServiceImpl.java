...
public class CallbackServiceImpl implements CallbackService {
	...
	/**
	 * This is a constructor which is used by the Callback Verticle to instantiate a
	 * RabbitMQ client.
	 * 
	 ...
	 */
	public CallbackServiceImpl(RabbitMQClient clientInstance, WebClient webClientInstance, JsonObject propObj,
			Vertx vertxInstance) {
	...
	/**
	 * <h1>This method perform</h1>
	 * <li>Call method "connectToCallbackNotificationQueue" and handle success and
	 * failure result.
	 * </li>
	 * 
	 ...
	 */
	@Override
	public CallbackService getMessageFromCallbackNotificationQueue(JsonObject request,
		...
		if (request != null && !request.isEmpty()) {
			...
			result.onComplete(resultHandler -> {
				if (resultHandler.succeeded()) {
					logger.info(Constants.SUCCESS + resultHandler.result());
					finalResponse.put(Constants.SUCCESS, Constants.CONNECT_TO_CALLBACK_NOTIFICATION_QUEUE);
					handler.handle(Future.succeededFuture(finalResponse));
				}
				if (resultHandler.failed()) {
					logger.error(Constants.ERROR + resultHandler.cause());
					finalResponse.put(Constants.FAILURE, Constants.CONNECT_TO_CALLBACK_NOTIFICATION_QUEUE_FAIL);
	 ...
	 * 
	 */
	private Future<JsonObject> connectToCallbackNotificationQueue(JsonObject request) {
		JsonObject finalResponse = new JsonObject();
		Promise<JsonObject> promise = Promise.promise();

		if (request != null && !request.isEmpty()) {
			/* Set Queue Options */
			QueueOptions options = new QueueOptions().setMaxInternalQueueSize(1000).setKeepMostRecent(true);
			...
			client.start(startHandler -> {
				if (startHandler.succeeded()) {
									...
									if (operation != null && !operation.isEmpty() && !operation.isBlank()) {
										if (operation.equals(Constants.CREATE) || operation.equals(Constants.UPDATE)
											...
											result.onComplete(resultHandler -> {
												if (resultHandler.succeeded()) {
													logger.info(
															Constants.DATABASE_QUERY_RESULT + resultHandler.result());
													finalResponse.put(Constants.DATABASE_QUERY_RESULT, Constants.CACHE_UPDATE_SUCCESS);
													promise.complete(finalResponse);
												} else {
													logger.error(Constants.DATABASE_QUERY_RESULT + Constants.COLON
															+ resultHandler.cause());
													finalResponse.put(Constants.DATABASE_QUERY_RESULT, Constants.DATABASE_QUERY_FAIL);
													promise.fail(finalResponse.toString());
												}
											});
										} else {
					...
					finalResponse.put(Constants.ERROR, Constants.QUEUE_CONNECTION_FAIL + Constants.COLON + queueName);
					promise.fail(finalResponse.toString());
				}
			});
		}
	 ...
	 * <p>
	 * <h1>This method execute tasks</h1>
	 * <li>Call method "connectToCallbackDataQueue" and handle success and failure
	 * result</li>
	 * </p>
	 * 
	 * @param request JSON Object
	 ...
	 */
	@Override
	public CallbackService getMessageFromCallbackDataQueue(JsonObject request,
			Handler<AsyncResult<JsonObject>> handler) {
		JsonObject finalResponse = new JsonObject();
		if (request != null && !request.isEmpty()) {
			...
			result.onComplete(resultHandler -> {
				if (resultHandler.succeeded()) {
					logger.info(Constants.SUCCESS + resultHandler.result());
					finalResponse.put(Constants.SUCCESS, Constants.CONNECT_TO_CALLBACK_DATA_QUEUE);
					handler.handle(Future.succeededFuture(finalResponse));
				}

				if (resultHandler.failed()) {
					logger.error(Constants.ERROR + resultHandler.cause());
					finalResponse.put(Constants.FAILURE, Constants.CONNECT_TO_CALLBACK_DATA_QUEUE_FAIL);
					handler.handle(Future.failedFuture(finalResponse.toString()));
				}
			});
		}
	 ...
	 * <li> Send message data to interested callbackUrl </li> 
	 * </p>
	 *
	 * @param request which is a JSON object
	 * @return response which is a Future object of promise of JSON type
	 * 
	 */
	private Future<JsonObject> connectToCallbackDataQueue(JsonObject request) {
				...
				if (startHandler.succeeded()) {
					...
					client.basicConsumer(queueName, options, rabbitMQConsumerAsyncResult -> {
						if (rabbitMQConsumerAsyncResult.succeeded()) {
							...
							mqConsumer.handler(message -> {
								...
								if (body != null) {
									...
									if (callBackJsonObj != null && !callBackJsonObj.isEmpty()) {
										...
										result.onComplete(resultHandler -> {
											if (resultHandler.succeeded()) {
												logger.info(Constants.CALLBACK_URL_RESPONSE + Constants.COLON + resultHandler.result());
												finalResponse.put(Constants.SUCCESS, Constants.DATA_SEND_TO_CALLBACK_URL_SUCCESS);
												promise.complete(finalResponse);
											} else {
										...
										logger.error(Constants.NO_CALLBACK_URL_FOR_ROUTING_KEY + Constants.COLON + routingKey);
										finalResponse.put(Constants.ERROR,
							...
							logger.error(Constants.ERROR + Constants.GET_QUEUE_MESSAGE_FAIL + Constants.COLON + queueName);
							finalResponse.put(Constants.ERROR, Constants.GET_QUEUE_MESSAGE_FAIL + queueName);
							promise.fail(finalResponse.toString());
						}
					});
				} else {
	 ...
	 * <p>
	 * <h1>This method execute tasks</h1>
	 * <li>Call method "sendDataToCallBackSubscriber"</li> 
	 * <li>Handle success and failure result</li>
	 * </p>
	 * 
	 * @param request JSON Object
	 * @param handler Handler to return response in case of success and appropriate
	 ...
	 */
	@Override
	public CallbackService sendDataToCallBackSubscriber(JsonObject _request, Handler<AsyncResult<JsonObject>> handler) {
		JsonObject finalResponse = new JsonObject();
		if (_request != null && !_request.isEmpty()) {
			Future<JsonObject> result = sendDataToCallBackSubscriber(_request);
			result.onComplete(resultHandler -> {
				if (resultHandler.succeeded()) {
				...
				if (resultHandler.failed()) {
					logger.error(Constants.ERROR + resultHandler.cause());
					finalResponse.put(Constants.FAILURE, Constants.DATA_SEND_TO_CALLBACK_URL_FAIL);
					handler.handle(Future.failedFuture(finalResponse.toString()));
				}
			});
		}
	...
	/*
	 * <p> <h1> This method execute tasks </h1> 
	 ...
	 * 
	 */
	private Future<JsonObject> sendDataToCallBackSubscriber(JsonObject request) {
		JsonObject finalResponse = new JsonObject();
		Promise<JsonObject> promise = Promise.promise();
		...
		if (request != null && !request.isEmpty()) {
			...
			JsonObject currentMessage_JsonObj = request.getJsonObject(Constants.CURRENT_MESSAGE_JSON_OBJECT);
			...
			try {
				if (callBackUrl != null && !callBackUrl.isEmpty() && !callBackUrl.isBlank()) {
					if (userName != null && password != null && !userName.isBlank() && !password.isBlank()) {
						webRequest = webClient.postAbs(callBackUrl.toString()).basicAuthentication(userName, password);
					} else {
					...
					if (webRequest != null) {
							...
							if (handler.succeeded()) {
								...
								if (result != null) {
									...
									if (status == HttpStatus.SC_OK) {
										...
										logger.info(Constants.RESPONSE_BODY + Constants.COLON + 
												Constants.NEW_LINE + responseBody);
						...
						logger.error(Constants.ERROR + Constants.COLON + Constants.CREATE_CALLBACK_REQUEST_OBJECT_FAIL);
	...
	private void deleteEntryFromCache(String _key) {
		pgCache.remove(_key);
	}
	 ...
	 * <p>
	 * <h1>This method execute tasks</h1>
	 * <li>Call method "connectToCallBackDataBase" and handle success and failure
	 * result</li>
	 * </p>
	 * 
	 * @param request JSON Object
	 ...
	 */
	@Override
	public CallbackService queryCallBackDataBase(JsonObject request, Handler<AsyncResult<JsonObject>> handler) {
		JsonObject finalResponse = new JsonObject();
		if (request != null && !request.isEmpty()) {
			...
			result.onComplete(resultHandler -> {
				if (resultHandler.succeeded()) {
					handler.handle(Future.succeededFuture(resultHandler.result()));
					logger.info(Constants.SUCCESS + resultHandler.result());
					finalResponse.put(Constants.SUCCESS, Constants.DATABASE_QUERY_SUCCESS);
					handler.handle(Future.succeededFuture(finalResponse));
				}
				if (resultHandler.failed()) {
					logger.error(Constants.ERROR + resultHandler.cause());
					finalResponse.put(Constants.FAILURE, Constants.DATABASE_QUERY_FAIL);
					handler.handle(Future.failedFuture(finalResponse.toString()));
				}
			});
		}
	 ...
	 * <p> 
	 * <h1> This method execute tasks </h1> 
	 * <li> Create instance of pgClient and query database</li> 
	 ...
	 * 
	 */
	private Future<JsonObject> queryCallBackDataBase(JsonObject request) {
		JsonObject finalResponse = new JsonObject();
		Promise<JsonObject> promise = Promise.promise();
		...
		if (connectOptions == null) {
			connectOptions = new PgConnectOptions().setPort(databasePort).setHost(databaseIP).setDatabase(databaseName)
