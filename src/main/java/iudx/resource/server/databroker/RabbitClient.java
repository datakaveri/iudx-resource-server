...
public class RabbitClient {
  ...
  Future<JsonObject> createQueue(JsonObject request, String vhost) {
    ...
    if (request != null && !request.isEmpty()) {
      ...
      arguments
          .put(Constants.X_MESSAGE_TTL_NAME, Constants.X_MESSAGE_TTL_VALUE)
                ...
                if (ar.succeeded()) {
                  ...
                  if (response != null && !response.equals(" ")) {
                    ...
                    if (status == HttpStatus.SC_CREATED) {
                      ...
                      finalResponse.mergeIn(
                          Util.getResponseJson(
                              HttpStatus.SC_CONFLICT, FAILURE, QUEUE_ALREADY_EXISTS),
                          true);
                  ...
                  if (ar.succeeded()) {
                    ...
                    if (response != null && !response.equals(" ")) {
                      ...
                      if (status == HttpStatus.SC_CREATED) {
                        ...
                        finalResponse.mergeIn(
                            Util.getResponseJson(status, FAILURE, QUEUE_EXCHANGE_NOT_FOUND));
                      }
                    }
                ...
                if (ar.succeeded()) {
                  ...
                  if (response != null && !response.equals(" ")) {
                    ...
                    if (status == HttpStatus.SC_CREATED) {
                      ...
                      finalResponse.mergeIn(
                          Util.getResponseJson(
                              HttpStatus.SC_CONFLICT, FAILURE, VHOST_ALREADY_EXISTS));
                    }
                  }
                  promise.complete(finalResponse);
                      ...
                      handler -> {
                        if (handler.succeeded()) {
                          ...
                          Future<JsonObject> createUserinDb =
                              createUserInDb(userid, Util.getSha(password));
