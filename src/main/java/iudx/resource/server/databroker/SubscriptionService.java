                                                ...
                                                userPermissionHandler -> {
                                                  if (userPermissionHandler.succeeded()) {
                                                    registerStreamingSubscriptionResponse.put(
                                                        Constants.USER_NAME, streamingUserName);
                                                    registerStreamingSubscriptionResponse.put(
                                                        Constants.APIKEY, apiKey);
                                                    registerStreamingSubscriptionResponse.put(
                                                        Constants.ID, queueName);
                                                    registerStreamingSubscriptionResponse.put(
                                                        Constants.URL, this.amqpUrl);
                                                    registerStreamingSubscriptionResponse.put(
                                                        Constants.PORT, this.amqpPort);
                                                    registerStreamingSubscriptionResponse.put(
                                                        Constants.VHOST, this.vhost);
                                  ...
                                  if (routingKey != null) {
                                    if (routingKey.isEmpty()
                                      ...
                                      if (isGroupResource(routingKey)) {
                                        exchangeName = routingKey;
                                        array.add(
                                            exchangeName + Constants.DATA_WILDCARD_ROUTINGKEY);
                                      } else {
                                        exchangeName =
                                ...
                                if (resultHandlerbind.succeeded()) {
                                  ...
                                  LOGGER.debug(
                                      "sucess :: totalBindSuccess "
                                          ...
                                          + resultHandlerbind.result());
                                  JsonObject bindResponse = (JsonObject) resultHandlerbind.result();
                                  if (bindResponse.containsKey(Constants.TITLE)
                                      && bindResponse
                                          .getString(Constants.TITLE)
                                          .equalsIgnoreCase(Constants.FAILURE)) {
                                    LOGGER.error("failed ::" + resultHandlerbind.cause());
                                    String deleteQuery =
                ...
                if (resultHandlerSelectID.succeeded()) {
                  ...
                  if (!subscriptionID.equalsIgnoreCase(subscriptionIDdb)
                    ...
                    deleteCallbackSubscriptionResponse.put(
                        Constants.ERROR, "Call Back ID not found");
                                ...
                                rabbitClient
                                    ...
                                    .basicPublish(
                                        ...
                                        resultHandler -> {
                                          if (resultHandler.succeeded()) {
                                            deleteCallbackSubscriptionResponse.put(
                                                Constants.SUBSCRIPTION_ID, subscriptionID);
                                            LOGGER.debug("Info : Message published to queue");
                                          } else {
                ...
                if (ar.succeeded()) {
                  ...
                  if (result.size() > 0) {
                    for (Row row : result) {
                      ...
                      listCallbackSubscriptionResponse.put(
                          Constants.SUBSCRIPTION_ID, subscriptionIDdb);
