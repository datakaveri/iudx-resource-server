          ...
          if (countHandler.succeeded()) {
            try {
              ...
              if (total == 0) {
                ...
                responseBuilder =
                    new ResponseBuilder(SUCCESS).setTypeAndTitle(200).setCount((int) total);
                handler.handle(Future.succeededFuture(responseBuilder.getResponse()));
              }
    ...
    result.onComplete(
        handlers -> {
          if (handlers.succeeded()) {
            ...
            if (jsonArray.size() == 0) {
              responseBuilder =
                  new ResponseBuilder(FAILED).setTypeAndTitle(204).setMessage("NO ID Present");
              handler.handle(Future.succeededFuture(responseBuilder.getResponse()));
            }
            cacheCall(jsonArray)
                .onSuccess(
  ...
  public Future<JsonArray> cacheCall(JsonArray jsonArray) {
    ...
    for (i = 0; i < jsonArray.size(); i++) {
      ...
      resourceCount.put(
          ...
          Integer.valueOf(jsonArray.getJsonObject(i).getString("count")));
