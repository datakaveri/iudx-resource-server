...
public class Constants {
  ...
  public static final String OVERVIEW_QUERY =
      "SELECT month,year,COALESCE(counts, 0) as counts\n"
          + "FROM  (\n"
          + "   SELECT day::date ,to_char(date_trunc('month', day),'FMmonth') as"
          + " month,extract('year' from day) as year\n"
  ...
  public static final String SUMMARY_QUERY_FOR_METERING =
      "select resourceid,count(*) from auditing_rs where (resourceid like '%/%/%/%' or resourceid"
          + " like '%/%/%/%/%') ";
