...
public class ParamsValidatorTest {
  ...
  @Mock MultiMap paramsMap;
  @Mock Future<List<String>> listFuture;
  @Mock AsyncResult<List<String>> listAsyncResult;
  ...
  public void test_validate(VertxTestContext vertxTestContext) {
    ...
    doAnswer(
            new Answer<AsyncResult<List<String>>>() {
              @Override
            ...
            handler -> {
              if (handler.failed()) {
                switch (value) {
                  case "TEMPORAL":
                    assertEquals(
                        "Temporal parameters are not supported by RS group/Item.",
                        handler.cause().getMessage());
                    break;
                  case "SPATIAL":
                    assertEquals(
                        "Spatial parameters are not supported by RS group/Item.",
                        handler.cause().getMessage());
                    break;
                  default:
                    assertEquals(
                        "Attribute parameters are not supported by RS group/Item.",
                        handler.cause().getMessage());
                    break;
                }
                vertxTestContext.completeNow();
  ...
  public void test_isValidCoordinatesForGeometry_with_DxRuntimeException(
      String geo, VertxTestContext vertxTestContext) {
    when(paramsMap.get(anyString()))
        .thenReturn(
            "Dummy/string/value",
            geo,
            "[{ \"Polygon\" : \"some_value\"},{ \"Point\" : \"some_value\"},{ \"LineString\" :"
                + " \"some_value\"},{ \"Polygon\" : \"some_value\"}]");
    when(catalogueService.getApplicableFilters(anyString())).thenReturn(listFuture);
    when(listFuture.result()).thenReturn(stringList);
    when(listAsyncResult.succeeded()).thenReturn(true);
    when(stringList.contains(anyString())).thenReturn(true);
