...
public class SubscriptionServiceTest {
  ...
  public void testCreateSubscriptionSuccess(VertxTestContext vertxTestContext) {
    ...
    service
        .createSubscription(json, databroker, pgService, authInfo, cacheService)
        .onComplete(
            handler -> {
  ...
  static Stream<Arguments> response() {
    return Stream.of(
        Arguments.of(
            "{ \"type\":409 }",
            "{\"type\":409,\"title\":\"Already exists\",\"detail\":\"Subscription Already"
                + " exists\"}"),
        Arguments.of(
  ...
  public void testCreateSubscriptionFailure(
      String type, String result, VertxTestContext vertxTestContext) {
  ...
  public void testUpdateSubscriptionSuccess(VertxTestContext vertxTestContext) {
    ...
    doAnswer(
        ...
        .executeQuery(anyString(), any());
    service
        .updateSubscription(json, databroker, pgService, authInfo)
        .onComplete(
            handler -> {
              if (handler.succeeded()) {
                JsonObject expected =
                    new JsonObject(
                        "{\"type\":\"urn:dx:rs:success\",\"title\":\"success\",\"results\":[{\"entities\":[\"Dummy"
                            + " entity\"]}]}\n");
        ...
        .onComplete(
            handler -> {
              if (handler.failed()) {
                ...
                String expected =
                    throwable
                        + "{\"type\":409,\"title\":\"Already exists\",\"detail\":\"Subscription"
                        + " Already exists\"}";
                assertEquals(expected, handler.cause().toString());
                vertxTestContext.completeNow();
              } else {
  ...
  public void testDeleteSubscriptionSuccess(VertxTestContext vertxTestContext) {
    ...
    doAnswer(
        ...
        .executeQuery(anyString(), any());
    service
        .deleteSubscription(json, databroker, pgService)
        .onComplete(
            handler -> {
              if (handler.succeeded()) {
                assertEquals(json, handler.result());
        ...
        .onComplete(
            handler -> {
              if (handler.failed()) {
                ...
                String expected =
                    throwable
                        + "{\"type\":404,\"title\":\"Not Found\",\"detail\":\"Resource not"
                        + " found\"}";
                assertEquals(expected, handler.cause().toString());
                vertxTestContext.completeNow();
              } else {
  ...
  public void testGetSubscriptionSuccess(VertxTestContext vertxTestContext) {
    ...
    doAnswer(
        ...
        .onComplete(any());
    service
        .getSubscription(json, databroker, pgService)
        .onComplete(
            handler -> {
              if (handler.succeeded()) {
                assertEquals(json, handler.result());
  ...
  public void testAppendSubscriptionSuccess(VertxTestContext vertxTestContext) {
    ...
    service
        .appendSubscription(json, databroker, pgService, authInfo)
        .onComplete(
            handler -> {
              if (handler.succeeded()) {
                assertEquals(json, handler.result());
  ...
  public void testAllSubscriptionQueue(VertxTestContext vertxTestContext) {
    ...
    JsonObject jsonObject =
        new JsonObject().put(USER_ID, "89a36273d77dac4cf38114fca1bbe64392547f86");
    when(asyncResult.succeeded()).thenReturn(true);
    when(asyncResult.result()).thenReturn(json);
    ...
    subscriptionService
        .getAllSubscriptionQueueForUser(jsonObject, pgService)
        .onComplete(
            handler -> {
              if (handler.succeeded()) {
