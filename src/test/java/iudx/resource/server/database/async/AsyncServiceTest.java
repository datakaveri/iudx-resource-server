...
public class AsyncServiceTest {
  ...
  public void testGetRecord4RequestIdFailure(VertxTestContext vertxTestContext) {
    ...
    asyncService2 =
        new AsyncServiceImpl(
            Vertx.vertx(), client, postgresService, fileOpsHelper, timeLimit, filePath);
    asyncService2
        .getRecord4RequestId("Dummy ID")
        .onComplete(
            handler -> {
              if (handler.failed()) {
                assertEquals(
                    "Record doesn't exist in db for requestId.", handler.cause().getMessage());
    ...
    asyncService2.scrollQuery(
        ...
        handler -> {
          if (handler.succeeded()) {
            ...
            assertEquals(
                "{\"type\":400,\"detail\":\"bad parameters\"}", handler.cause().getMessage());
            vertxTestContext.completeNow();
          }
        });
  }
  ...
  public void testUpdateProgressFailure(boolean value, VertxTestContext vertxTestContext) {
    AsyncFileScrollProgressListener.executionCounter =
        mock(AsyncFileScrollProgressListener.ExecutionCounter.class);
  ...
  //                    @SuppressWarnings("unchecked")
  //                    @Override
  //                    public AsyncResult<JsonObject> answer(InvocationOnMock arg0) throws
  // Throwable {
  //                      ((Handler<AsyncResult<JsonObject>>)
  // arg0.getArgument(2)).handle(asyncResult1);
  //                      return null;
  //                    }
  //                  })
  //          .when(fileOpsHelper)
