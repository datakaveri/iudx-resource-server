...
public class LatestServiceTest {
  ...
  static void init(Vertx vertx, VertxTestContext testContext) {
    ...
    new RedisClient(vertx, redisConfig)
        .start()
        .onSuccess(
            handler -> {
              redisClient = handler;
  ...
  void searchLatestResourceflood(VertxTestContext testContext) {
    ...
    JsonObject request =
        new JsonObject().put("id", new JsonArray().add(id)).put("searchType", "latestSearch");
        ...
        testContext.failing(
            response ->
                testContext.verify(
                    () -> {
                      assertEquals(
                          "No id found", new JsonObject(response.getMessage()).getString("detail"));
                      testContext.completeNow();
                    })));
  }
  ...
  void searchEmptyId(VertxTestContext testContext) {
    JsonObject request = new JsonObject().put("id", new JsonArray()).put("options", "id");
