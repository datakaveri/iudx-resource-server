...
public class EncryptionServiceImplTest {
  ...
  public void encodePublicKeyForInvalidPublicKey(
      JsonObject jsonObject, VertxTestContext vertxTestContext) {
    encryptionService
        .encodePublicKey(jsonObject)
        .onComplete(
            handler -> {
  ...
  static Stream<Arguments> publicKeyValues() {
    return Stream.of(
        ...
        Arguments.of(
            new JsonObject().put("dummyKey", "1nSR-jsOD2xIDJvge1PNZrxwXdUChrv4C1cs2UJ1ew8=")));
  }
  ...
  static Stream<Arguments> invalidPublicKey() {
    return Stream.of(
        Arguments.of(
            new JsonObject().put(ENCODED_KEY, "I6W24wZTKcH0Xl6ykwD8eSLv8EZIhPU2WkwYzmZzP10=="),
            "Exception while decoding public key: java.lang.IllegalArgumentException: Input byte"
                + " array has incorrect ending byte at 44"),
        Arguments.of(
            new JsonObject().put(ENCODED_KEY, "I6W24wZTKcH0Xl6ykwD8eS+v8EZIhPU2WkwYzmZzP10"),
            "Exception while decoding public key: java.lang.IllegalArgumentException: Illegal"
                + " base64 character 2b"),
        Arguments.of(
            new JsonObject().put(ENCODED_KEY, "I6W24wZTKcH0Xl6ykwD8eS/v8EZIhPU2WkwYzmZzP10"),
            "Exception while decoding public key: java.lang.IllegalArgumentException: Illegal"
        ...
        Arguments.of(
            new JsonObject()
                .put(
                    ENCODED_KEY,
                    "I6W24wZTKcaaaaaa11111...........11111111111aaaaaaav8EZIhPU2WkwYzmZzP10"),
            "Exception while decoding public key: java.lang.IllegalArgumentException: Illegal"
                + " base64 character 2e"));
  }
  ...
  public void testEncryptForInvalidMessage(
      String message, String expected, VertxTestContext vertxTestContext) {
    encryptionService
        .encrypt(message, new JsonObject())
        .onComplete(
            handler -> {
              if (handler.succeeded()) {
  ...
  public void testEncryptForInvalidPublicKey(
      JsonObject jsonObject, String expected, VertxTestContext vertxTestContext) {
    String message = "Dummy string to be encrypted";
    encryptionService
        .encrypt(message, jsonObject)
        .onComplete(
            handler -> {
              if (handler.succeeded()) {
                vertxTestContext.failNow("test succeeded for invalid message");
  ...
  public void testEncryptFailure(VertxTestContext vertxTestContext) throws SodiumException {
    ...
    encryptionService
        .encrypt("Dummy message", jsonObject)
        .onComplete(
            handler -> {
              if (handler.succeeded()) {
                vertxTestContext.failNow(handler.cause().getMessage());
                ...
                assertEquals(
                    "Sodium exception: com.goterl.lazysodium.exceptions.SodiumException",
        ...
        .onComplete(
            handler -> {
              if (handler.succeeded()) {
                ...
                encryptionService
                    .decrypt(encodedCipherText, jsonObject)
                    .onComplete(
                        decryptHandler -> {
                          if (decryptHandler.succeeded()) {
                            ...
                            vertxTestContext.failNow(
                                "Failure in decryption: " + decryptHandler.cause().getMessage());
                          }
                        });
              } else {
                vertxTestContext.failNow(handler.cause().getMessage());
  ...
  public void testDecryptWithInvalidMessage(
      String encodedCipherText, VertxTestContext vertxTestContext) {
    encryptionService
        .decrypt(encodedCipherText, jsonObject)
        .onComplete(
            handler -> {
  ...
  static Stream<Arguments> encodedCipherText() {
    return Stream.of(
        Arguments.of(
            "RjU4MTYwRTkxNzc2NkFFMzl==",
            "IllegalArgumentException: java.lang.IllegalArgumentException: Input byte array has"
                + " incorrect ending byte at 24"),
        Arguments.of(
            "RjU4MTYwRTkxNzc2NkFFMzl", "Exception : java.lang.NegativeArraySizeException: -31"),
        Arguments.of(
            "encodedCipherText",
            "IllegalArgumentException: java.lang.IllegalArgumentException: Last unit does not have"
                + " enough valid bits"),
        Arguments.of(
            "RjU4MTYwRTkxNzc2NkFFMzlFNjg0MURCN0NENTM2Q0EyNzUxODAxOTA4NzQ5MzgyRTAzRTMxRjlENkYzNDYxM0FFMUU4RUExQUZENzM1OTU2ODcwMDQxNjdCNkYyQ0RCNDNDOTA5MkM1ODE3RDExMUU0RkNBQjhGM0NFMURCQjg4RjE3OUYxNjI3NTk3NTU5OTJFMzk4OTg=",
            "Sodium Exception"),
        Arguments.of("abcdefgh", "Exception : java.lang.NegativeArraySizeException: -42"),
        Arguments.of("abcd", "Exception : java.lang.NegativeArraySizeException: -45"));
  }
  ...
  public void testDecryptFailure(
      String encodedCipherText, String expected, VertxTestContext vertxTestContext) {
    jsonObject.put("keyPair", keyPair);
    encryptionService
        .decrypt(encodedCipherText, jsonObject)
        .onComplete(
